...
function signJwt(payload:any){const h=b64(JSON.stringify({alg:'HS256',typ:'JWT'})),b=b64(JSON.stringify(payload)),s=crypto.createHmac('sha256',secret).update(`${h}.${b}`).digest('base64url');return`${h}.${b}.${s}`}
function verifyJwt(token:string){try{const[h,b,s]=token.split('.');if(!h||!b||!s)return null;const e=crypto.createHmac('sha256',secret).update(`${h}.${b}`).digest('base64url');if(s.length!==e.length||!crypto.timingSafeEqual(Buffer.from(s),Buffer.from(e)))return null;const p=JSON.parse(Buffer.from(b,'base64url').toString('utf8'));if(!p.exp||Date.now()/1000>=p.exp)return null;return p}catch{return null}}
...
async function verifyPassword(password:string,stored:string){try{const[salt]=stored.split(':');const fresh=await scryptHash(password,salt);return fresh.length===stored.length&&crypto.timingSafeEqual(Buffer.from(fresh),Buffer.from(stored))}catch{return false}}
...
function externalOrigin(req:express.Request){const proto=String(req.headers['x-forwarded-proto']||req.protocol).split(',')[0].trim(),host=String(req.headers['x-forwarded-host']||req.get('host')||'').split(',')[0].trim();return`${proto}://${host}`}
function checkOrigin(req:express.Request,res:express.Response){if(['GET','HEAD','OPTIONS'].includes(req.method))return true;const origin=req.headers.origin;if(!origin)return true;if(origin!==externalOrigin(req)){res.status(403).json({error:'origin_rejected'});return false}return true}
...
app.use(async(req,res)=>{if(!checkOrigin(req,res))return;const u=(req as any).relayosUser||await currentUser(req);const url=`http://127.0.0.1:${internalPort}${req.originalUrl}`;const headers:any={};for(const[k,v]of Object.entries(req.headers)){if(v!==undefined&&!['host','content-length','cookie','x-relayos-operator-id'].includes(k))headers[k]=Array.isArray(v)?v.join(','):v}if(u)headers['x-relayos-operator-id']=u.id;let body:any=undefined;if(!['GET','HEAD'].includes(req.method)&&req.body!==undefined){headers['content-type']='application/json';body=JSON.stringify(req.body)}try{const r=await fetch(url,{method:req.method,headers,body,redirect:'manual'});r.headers.forEach((v,k)=>{if(!['transfer-encoding','content-length','set-cookie'].includes(k.toLowerCase()))res.setHeader(k,v)});res.status(r.status);res.send(Buffer.from(await r.arrayBuffer()))}catch(e:any){res.status(502).json({error:'backend_unavailable',detail:e.message})}});
